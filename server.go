...
func (s *Server) cleanupOldGames() {
	...
	for id, gh := range s.games {
		gh.mu.Lock()
		if gh.g.WinningTeam != nil && gh.g.CreatedAt.Add(2 * 168 * time.Hour).Before(time.Now()) {  // 2 weeks
			delete(s.games, id)
			log.Printf("Removed completed game %s\n", id)
		} else if gh.g.CreatedAt.Add(26 * 168 * time.Hour).Before(time.Now()) {  // 6 months
			delete(s.games, id)
